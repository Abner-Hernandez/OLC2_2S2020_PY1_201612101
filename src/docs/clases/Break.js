...
class Break{
    ...
    operate(tab) {
        if (this.used) {
            var ret = new Value("", null, Type.BREAK, this.row, this.column);
            ret.used = false;
            return ret;
        } else {
            try{ add_error_E( {error: "La Instruccion " + this.type_exp + " No puede estar fuerda de un ciclo FOR, WHILE, DO WHILE, BREAK.", type: 'SEMANTICO', line: this.row, column: this.column} ); }catch(e){}
